...
func ProcessBlocks(uuid string, results *api.ResultsWrapper, outputPath, pageName string, apiClient api.NotionAPI, bearerToken string, processedBlocks map[string]string) {
	...
	for _, block := range results.Results {
		...
		switch block.Type {
		case "child_page":
			processChildPageBlock(&block, apiClient, bearerToken, processedBlocks, linkTitles)
		case "link_to_page":
			processLinkToPageBlock(&block, apiClient, bearerToken, processedBlocks, linkTitles)
		}
...
func processLinkToPageBlock(block *api.Block, apiClient api.NotionAPI, bearerToken string, processedBlocks, linkTitles map[string]string) {
...
func processChildPageBlock(block *api.Block, apiClient api.NotionAPI, bearerToken string, processedBlocks, linkTitles map[string]string) {
